    ...
    <div className="bg-card border border-border rounded-xl shadow-sm hover:shadow-md transition-shadow flex flex-col overflow-hidden">
      ...
      <div className="p-5 pb-3">
        ...
        <h3 className="text-lg font-bold leading-tight mb-2">{post.title}</h3>
                 ...
                 <div className="flex items-center gap-4 text-[10px] text-slate-500 font-semibold mb-4 bg-slate-50 p-2 rounded-lg border border-slate-100">
                    <div className="flex items-center gap-1.5"><BarChart2 className="w-3 h-3 text-blue-500" /> Analytics Summary:</div>
        ...
        <div className="flex flex-wrap gap-2 mb-1">
           {post.complexity && (
             <span className="px-2 py-0.5 rounded text-[10px] font-bold tracking-wide bg-slate-100 text-slate-600 uppercase">
               {post.complexity}
             </span>
           )}
           {post.tags?.map(tag => (
             <span key={tag} className="px-2 py-0.5 rounded bg-blue-50 text-blue-600 text-[11px] font-medium border border-blue-100">
               #{tag}
             </span>
           ))}
        </div>
      ...
      <div className="w-full border-y border-border relative group flex items-start min-h-[150px] max-h-[300px] overflow-hidden">
        {post.type === "Code" ? (
           <div className="w-full h-full p-4 overflow-hidden bg-slate-900 flex-1">
          ...
          <img src={post.preview} alt="Post preview" className="w-full min-h-[150px] max-h-[300px] object-cover bg-slate-100" />
        ) : (
          <div className="flex items-center justify-center w-full min-h-[150px] bg-slate-50">
             {getTypeIcon(post.type)}
          </div>
        )}
      </div>
      ...
      <div className="px-4 py-3 bg-slate-50/50 flex items-center justify-between">
        <div className="flex items-center gap-1">
          <Button
            ...
            size="sm"
            onClick={handleLike}
            className={`gap-1.5 rounded-full ${isLiked ? "text-red-500 hover:text-red-600 hover:bg-red-50" : "text-slate-500 hover:bg-slate-200"}`}
          >
          ...
          <Button
            ...
            className="gap-1.5 rounded-full text-slate-500 hover:bg-slate-200"
          >
          ...
          <Button
            ...
            size="sm"
            onClick={handleFork}
            className="gap-1.5 rounded-full text-slate-500 hover:text-blue-600 hover:bg-blue-50"
            title="Fork Project"
          >
      ...
      {/* Comments Drawer */}
      {showComments && (
        <div className="px-4 py-3 border-t border-border bg-slate-50 flex flex-col gap-3">
           <div className="flex gap-2">
              <input
                type="text"
                placeholder={replyToId ? "Write a reply..." : "Add a comment..."}
                className="flex-1 text-sm px-3 py-1.5 rounded-md border border-border focus:outline-none focus:ring-1 focus:ring-primary bg-white transition-shadow"
                value={commentText}
                    ...
                    <div key={comment.id} className="flex flex-col gap-1.5">
                       <div className="flex items-start gap-2.5">
                           <div className="w-7 h-7 rounded-full bg-slate-200 flex items-center justify-center shrink-0 shadow-sm border border-white">
                               <span className="text-[10px] font-bold text-slate-600">{commentAuthor?.username.charAt(0).toUpperCase()}</span>
                           </div>
                           <div className="bg-white px-3 py-2 rounded-xl rounded-tl-none border border-slate-200 shadow-sm flex-1">
                               <div className="flex justify-between items-baseline mb-0.5">
                                 <span className="font-semibold text-xs text-slate-800">{commentAuthor?.username}</span>
                               </div>
                               <p className="text-slate-600 text-[13px] leading-snug">{comment.text}</p>
                               <div className="flex items-center gap-3 mt-1.5">
                                 <button onClick={() => upvoteComment(post.id, comment.id)} className="flex items-center gap-1 text-[10px] sm:text-[11px] font-semibold text-slate-500 hover:text-emerald-500 transition-colors">
                                    <ThumbsUp className="w-3 h-3" /> {(comment as any).upvotes || 0} Upvotes
                                 </button>
                                 <button onClick={() => setReplyToId(comment.id)} className="flex items-center gap-1 text-[10px] sm:text-[11px] font-semibold text-slate-500 hover:text-blue-500 transition-colors">
                       ...
                       {comment.replies?.length > 0 && (
                          <div className="ml-10 space-y-2 mt-1 relative before:content-[''] before:absolute before:-left-3.5 before:top-0 before:w-px before:h-full before:bg-slate-200">
                             {comment.replies.map((reply) => {
                               const replyAuthor = users.find(u => u.id === reply.userId);
                               return (
                                 <div key={reply.id} className="flex items-start gap-2 relative">
                                     <div className="absolute -left-3.5 top-2.5 w-3 h-px bg-slate-200" />
                                     <div className="w-5 h-5 rounded-full bg-slate-200 flex items-center justify-center shrink-0 border border-white shadow-sm">
                                         <span className="text-[8px] font-bold text-slate-600">{replyAuthor?.username.charAt(0).toUpperCase()}</span>
                                     </div>
                                     <div className="bg-white px-2.5 py-1.5 rounded-lg rounded-tl-none border border-slate-200 flex-1 shadow-sm">
                                         <span className="font-semibold text-[11px] block text-slate-800 mb-0.5">{replyAuthor?.username}</span>
                                         <p className="text-slate-600 text-xs leading-snug">{reply.text}</p>
                                         <div className="flex items-center gap-3 mt-1.5">
                                            <button onClick={() => upvoteComment(post.id, reply.id)} className="flex items-center gap-1 text-[10px] sm:text-[11px] font-semibold text-slate-500 hover:text-emerald-500 transition-colors">
